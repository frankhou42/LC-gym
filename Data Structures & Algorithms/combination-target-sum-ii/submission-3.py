...
we are trying to get to the leaf node which is the possibilities
"""
class Solution:
...
#time complexity O(2^n * n) since there are 2^n probabilities (leaf nodes), 
#at each leaf node we do  O(n) opeartion to do a deep copy. 
#space complexity O(n) since there are 